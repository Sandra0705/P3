...
\begin{enumerate}
    ...
    \item[] \textbf{Bevis for punkt 2}.\\ 
       ...
       % Da normen af en vektor er summen af alle elementerne kvadreret, skal alle elementerne være lig 0 for at summen er lig 0.
...
På samme måde som normen til en vektor kan bestemmes, kan en norm til matricer også bestemmes. Denne norm kaldes for Frobenius normen eller Hilbert-Schmidt normen, og er defineret som følgende.

\begin{defn} \textbf{Frobenius normen}\\
    Lad $A = \begin{bmatrix}
        a_{ij}
    \end{bmatrix} \in \R^{n\times n}$. \textit{Frobenius normen} af $A$ er defineret ved
    %
    \begin{align}
      \norm{A}_F = \sqrt{ \sum_{i=1}^n \sum_{i=1}^n \abs{a_{ij}}^2 }
    \end{align}
\end{defn}

\autoref{sæt:frobenius_egenskab_1} og \autoref{sæt:frobenius_egenskab_2} er egenskaber tilhørende normer, som vil blive brugt fremadrettet i dette projekt. 

\begin{thmx}\textbf{}\\
\label{sæt:frobenius_egenskab_1}
    Lad $A\in \R^{n\times n}$ og $\textbf{x}\in \R^n$. Det gælder, at
    \begin{align*}
        \norm{A\textbf{x}}\leq \norm{A}_F \norm{\textbf{x}}
    \end{align*}
\end{thmx}
