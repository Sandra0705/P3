...
\begin{equation*}
    f(x)=q(t), \quad t \in I
\end{equation*}
hvor $q : I \rightarrow \R$ er en funktion, der afhænger af $t$, og $x : I \rightarrow \R$ er den ukendte funktion. $f : C^n(\R) \rightarrow C^0(\R)$ er en lineær afbildning fra definitionsmængden $C^n(\R)$ til værdimængden $C^0(\R)$, hvor $n \geq 0$ betegner differentialligningens orden. Når $f$ er en lineær afbildning, vil differentialligningen være lineær. \\

\begin{minipage}\textwidth
\begin{defn}\textbf{Lineære differentialligninger}
\newline
En differentialligning $f(x) = q(t)$ er lineær, hvis funktionens venstreside $f(x)$ opfylder begge linearitetsbetingelser:
\begin{itemize}
    \item[1.] $f(x_1+x_2)=f(x_1)+f(x_2)$
    \item[2.]$f(c\cdot x) = c \cdot f(x)$,
\end{itemize}
altså hvis $f : C^n(\R) \rightarrow C^0(\R)$ er en lineær afbildning, hvor $n$ angiver differentialligningens orden. $x, x_1, x_2$ er løsninger til differentialligningen, og $c \in \R$ er en skalar.
\end{defn}
\end{minipage}
